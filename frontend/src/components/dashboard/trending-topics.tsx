                ...
                <div className="space-y-0.5">
                  {rising.map((topic) => (
                    <div
                      ...
                      className="flex items-center gap-3 py-2 px-2 -mx-2 rounded-[var(--r)] hover:bg-[var(--paper-2)]/60 transition-colors"
                    >
                      <span className="text-[var(--forest)] text-sm font-medium shrink-0">
                        {"\u2191"}
                      </span>
                      <Link
                        href={`/research?q=${encodeURIComponent(topic.name)}`}
                        className="text-sm text-[var(--ink)] flex-1 min-w-0 truncate hover:underline hover:text-[#223a5e] transition-colors"
                      >
                        {topic.name}
                      </Link>
                      <Badge
                      ...
                      <span className="text-xs font-semibold text-[var(--forest)] tabular-nums shrink-0 w-12 text-right">
                        {formatGrowthRate(topic.growthRate)}
                      </span>
                      <span className="text-xs text-[var(--ink-4)] tabular-nums shrink-0 w-8 text-right">
                        {topic.recentCount}
                      </span>
                    </div>
                  ))}
                ...
                <div className="space-y-0.5">
                  {declining.map((topic) => (
                    <div
                      ...
                      className="flex items-center gap-3 py-2 px-2 -mx-2 rounded-[var(--r)] hover:bg-[var(--paper-2)]/60 transition-colors"
                    >
                      <span className="text-[var(--rust)] text-sm font-medium shrink-0">
                        {"\u2193"}
                      </span>
                      <Link
                        href={`/research?q=${encodeURIComponent(topic.name)}`}
                      ...
                      <span className="text-xs font-semibold text-[var(--rust)] tabular-nums shrink-0 w-12 text-right">
                        {formatGrowthRate(topic.growthRate)}
                      </span>
                      <span className="text-xs text-[var(--ink-4)] tabular-nums shrink-0 w-8 text-right">
                        {topic.recentCount}
